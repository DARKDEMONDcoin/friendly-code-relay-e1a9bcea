...
export async function buildApp(input: { prompt: string; user_id: string }): Promise<ToolResult> {
  ...
  try {
    ...
    return { ok: true, data: { project_id: projectId, build_url: `/build/${projectId}/chat`, note: "Project created. Open the build page to start coding." } };
  } catch (e) {
    return { ok: false, error: String(e) };
  }
}
