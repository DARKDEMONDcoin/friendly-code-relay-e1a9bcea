...
export async function ensureProject(args: {
  ...
  const probe = await fetch(`${CF_API}/accounts/${args.accountId}/pages/projects/${args.name}`, {
    headers: auth,
  });
  if (probe.status === 200) {
  ...
  // Create it.
  const create = await fetch(`${CF_API}/accounts/${args.accountId}/pages/projects`, {
    method: "POST",
...
export async function deployToPages(args: {
  ...
  const missingHashes = await checkMissing(
    jwt,
  ...
  const url = args.production === false ? dep.url : `https://${proj.subdomain}`;
