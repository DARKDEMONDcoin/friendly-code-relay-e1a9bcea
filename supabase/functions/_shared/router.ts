  ...
  function: {
    ...
    parameters: {
      ...
      properties: {
        ...
        mood: {
          type: "string",
          enum: ["neutral", "happy", "frustrated", "urgent", "curious", "sad", "excited"],
        },
        language: { type: "string", description: "ISO code: ar, en, fr, es..." },
        dialect: {
          type: "string",
          description: "egyptian, gulf, levantine, maghrebi, msa, american, british, or 'none'",
        },
        persona: {
          type: "string",
          enum: ["default", "friend", "teacher", "expert", "comedian"],
          description: "Best matching persona for this message",
        },
        tools_needed: {
          type: "array",
          items: { type: "string" },
          description:
            "List from: WEB_SEARCH, WIKIPEDIA, ARXIV, GITHUB_SEARCH, REDDIT_SEARCH, STACKOVERFLOW, NEWS_SEARCH, YOUTUBE_TRANSCRIPT, MATH_SOLVER, GENERATE_IMAGE, GENERATE_VIDEO, GENERATE_VOICE, FETCH_URL",
        },
        needs_memory: {
          type: "boolean",
          description: "True if user references past info or shares personal facts to remember",
        },
        needs_reasoning: {
          type: "boolean",
          description: "True for hard logic/math/multi-step problems",
        },
        needs_citations: {
          type: "boolean",
          description: "True if answer should cite sources (factual claims, news, research)",
        },
      },
      required: [
        "intent",
...
- needs_citations: true for factual claims that benefit from sources`;
  ...
  try {
    ...
    const endpoint = typeof target === "string" ? OPENROUTER_URL : target.url;
    const headers =
      typeof target === "string"
        ? { Authorization: `Bearer ${target}`, "Content-Type": "application/json" }
        : {
            ...(target.headers || { Authorization: `Bearer ${target.key}` }),
            "Content-Type": "application/json",
          };
    const model = typeof target === "string" ? ROUTER_MODEL : target.model || ROUTER_MODEL;
    ...
    const res = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          ...(recentContext
            ? [{ role: "user", content: `Recent context: ${recentContext.slice(0, 400)}` }]
            : []),
...
export function buildPersonaPrompt(
  ...
  const persona =
    userPreferredPersona && userPreferredPersona !== "default"
      ? userPreferredPersona
      : decision.persona;

  const personaPrompts: Record<string, string> = {
    friend:
      "Speak like a close, casual friend. Use the user's slang, be warm, supportive, and brief.",
    teacher:
      "Explain step by step like a patient teacher. Use simple analogies, check understanding, and offer examples.",
    expert:
      "Reply with technical precision, named concepts, and concise structured explanations. No fluff.",
    comedian:
      "Be playful and witty. Add light humor and clever wordplay. Stay helpful underneath the jokes.",
    default: "",
  };

  const moodAdjustments: Record<string, string> = {
    frustrated:
      "The user sounds frustrated — be calm, empathetic, and solution-focused. Acknowledge their feeling first, then help.",
  ...
  if (decision.dialect && decision.dialect !== "auto" && decision.dialect !== "none") {
    parts.push(
      `DIALECT LOCK: Reply in ${decision.dialect} dialect — match the user's exact register.`,
    );
  }
  return parts.length ? `\n\n--- ROUTING ADJUSTMENTS ---\n${parts.join("\n")}\n` : "";
}
