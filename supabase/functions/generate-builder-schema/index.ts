...
import { getLLM, ROUTER_MODELS, lovableEquivalent } from "../_shared/llm-router.ts";
import { getAuthUser } from "../_shared/auth.ts";
import { corsHeaders } from "../_shared/cors.ts";

const LOVABLE_URL = "https://ai.gateway.lovable.dev/v1/chat/completions";
const MODEL = ROUTER_MODELS.docs; // mapped per provider (Alibaba Qwen primary)
...
serve(async (req) => {
  ...
  try {
    ...
    // Primary: Alibaba/Qwen (via getLLM). Fallback: Lovable Gateway (gemini equivalent).
    const llm = await getLLM();
    const target = llm
      ? { url: llm.url, key: llm.key, model: llm.mapModel(MODEL) }
