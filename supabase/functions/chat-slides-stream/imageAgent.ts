...
function cacheKey(source: string, q: string): string {
  return `${source}::${q.toLowerCase()}`;
}

async function withTimeout<T>(p: Promise<T>, ms = TIMEOUT_MS): Promise<T | null> {
  return await Promise.race([p, new Promise<null>((res) => setTimeout(() => res(null), ms))]).catch(
    () => null,
  );
}
...
async function searchSerper(q: string): Promise<Candidate[]> {
  ...
  try {
    ...
    const list = (d?.images || []) as Array<{
      imageUrl?: string;
      title?: string;
      imageWidth?: number;
      imageHeight?: number;
    }>;
    const out = applyQualityFilters(
      list
...
async function searchFirecrawl(q: string): Promise<Candidate[]> {
  ...
  try {
    ...
    if (!r.ok) {
      console.warn("[firecrawl]", r.status);
      searchCache.set(ck, []);
      return [];
    }
    const d = await r.json();
    const list = (d?.data?.images || d?.images || []) as Array<{
      imageUrl?: string;
      url?: string;
      title?: string;
      imageWidth?: number;
...
function orientationForLayout(layout?: string, type?: string): "landscape" | "portrait" | "square" {
  ...
  if (t === "cover" || l === "image-full" || l === "hero" || l === "cover" || l.includes("full"))
    return "landscape";
  if (l === "split-left" || l === "split-right" || l === "split" || l.includes("portrait"))
    return "portrait";
...
async function aiJSON<T>(
  ...
  model = ROUTER_MODELS.slidesVision,
): Promise<T | null> {
  ...
  try {
    const r = await fetchWithAbort(
      router.url,
      {
        ...
        body: JSON.stringify({
          model: router.mapModel(model || ROUTER_MODELS.slidesVision),
          messages,
...
async function generateQueries(
  slide: SlideCtx,
  topic: string,
): Promise<{ queries: string[]; subject: string }> {
...
async function pickBestWithVision(
  slide: SlideCtx,
  topic: string,
  subject: string,
...
export async function smartImage(
  ...
  try {
    ...
    const queries = Array.from(new Set([...(aiQueries || []), baseQuery].filter(Boolean))).slice(
      0,
      5,
    );
    ...
    const fresh =
      usedUrls && usedUrls.size > 0 ? candidates.filter((c) => !usedUrls.has(c.url)) : candidates;
