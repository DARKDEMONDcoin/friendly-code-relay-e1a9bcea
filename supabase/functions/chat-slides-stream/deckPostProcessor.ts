...
function detectContentShape(s: Slide): {
  ...
  if (s.type === "cover")
    return { contentType: "title", bulletsCount, bodyWords, statsCount, hasImage };
  if (s.type === "section")
    return { contentType: "section", bulletsCount, bodyWords, statsCount, hasImage };
  if (s.type === "closing")
    return { contentType: "title", bulletsCount, bodyWords, statsCount, hasImage };
  if (s.type === "quote" || s.quote)
    return { contentType: "quote", bulletsCount, bodyWords, statsCount, hasImage };
  if (statsCount >= 3 || s.big_value)
    return { contentType: "stats", bulletsCount, bodyWords, statsCount, hasImage };
  if (Array.isArray(s.steps) && s.steps.length >= 2)
    return { contentType: "process", bulletsCount, bodyWords, statsCount, hasImage };
  if (Array.isArray(s.events) && s.events.length >= 2)
    return { contentType: "timeline", bulletsCount, bodyWords, statsCount, hasImage };
  if (
    (Array.isArray(s.left_bullets) && s.left_bullets.length) ||
    (Array.isArray(s.right_bullets) && s.right_bullets.length)
  ) {
  ...
  if (bulletsCount >= 3)
    return { contentType: "bullets", bulletsCount, bodyWords, statsCount, hasImage };
...
function pickLayout(
  s: Slide,
  shape: ReturnType<typeof detectContentShape>,
  recent: string[],
  seed: number,
): string {
...
function maybeSplit(s: Slide): Slide[] {
  ...
  if (
    layout &&
    ["pull-quote", "manifesto", "callout", "poster-typo", "big-number"].includes(layout)
  )
    return [s];
  ...
  if (bullets.length >= 6) {
    ...
    const a: Slide = {
      ...s,
      bullets: bullets.slice(0, half),
      title: `${s.title ?? ""} — 1/2`.trim(),
    };
...
function slideDensity(s: Slide): "airy" | "balanced" | "dense" {
  const total =
    wc(s.title as string) +
    wc(s.subtitle as string) +
    wc(s.body as string) +
...
export function postProcessDeck(slides: Slide[], topicSeedStr = ""): Slide[] {
  ...
  for (const s of slides) {
    ...
    if (
      IMAGE_LAYOUTS.includes(chosen) &&
      !s.image &&
      !(Array.isArray(s.images) && s.images.length)
    ) {
      const fallback =
        TEXT_LAYOUT_FALLBACKS.find((l) => !recentLayouts.slice(-2, -1).includes(l)) || "centered";
      s.layout = fallback;
  ...
  for (const s of final) {
    ...
    if (layout && recent2.slice(-2).includes(layout)) {
      const shape = detectContentShape(s);
      const alts = seededShuffle(
        SHAPE_TO_LAYOUTS[shape.contentType] ?? SHAPE_TO_LAYOUTS.text,
        seed + recent2.length,
      );
...
export function pickThemeForTopic(topic: string, language: string): string {
  ...
  if (
    /\b(finance|invest|bank|stock|crypto|fintech|profit|revenue|valuation|earnings|market)\b/.test(
      t,
    )
  )
    return "stripe";
  if (
    /\b(engineer|developer|api|saas|product|infra|backend|software|code|cloud|devops|platform)\b/.test(
      t,
    )
  )
    return "linear";
  if (/\b(apple|ios|iphone|mac|keynote|design system|ux|ui|sf pro)\b/.test(t))
    return "apple-keynote";
  if (/\b(ai|gpt|llm|ml|model|neural|automation|agent)\b/.test(t)) return "megsy-landing";
  if (/\b(fashion|magazine|editorial|art|culture|cinema|film|poetry|literature)\b/.test(t))
    return "editorial";
  if (/\b(luxury|gold|premium|exclusive|boutique|haute|estate|villa)\b/.test(t))
    return "dark-luxury";
  if (/\b(kids|child|baby|cute|fun|happy|spring|pastel|bakery|gift)\b/.test(t))
    return "pastel-minimal";
...
export function planDeckShape(opts: {
  audience?: string;
  durationMin?: number;
  requested?: number;
}): {
  ...
  if (opts.requested && opts.requested > 0) {
    return {
      slideCount: opts.requested,
      density: opts.requested > 25 ? "dense" : opts.requested > 14 ? "medium" : "light",
    };
  }
