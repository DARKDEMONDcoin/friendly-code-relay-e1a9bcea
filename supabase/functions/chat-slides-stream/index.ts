...
// Standard-tier requests are routed to the `plusai-presentation` edge function
// (Plus AI Presentations API → real .pptx). This streaming pipeline only handles
// the rich premium React-deck templates listed in REACT_TEMPLATES above.
...
async function aiJson<T = unknown>(
  messages: Array<{ role: string; content: string }>,
  model?: string,
): Promise<T | null> {
  const router = await getLLM();
  const chosen = router
    ? router.mapModel(model || ROUTER_MODELS.slides)
...
async function streamNarrative(
  ...
  const target = router
    ? {
        url: router.url,
        key: router.key,
        model: router.mapModel(model || ROUTER_MODELS.slidesNarrate),
      }
    : null;
  if (!target) throw new Error("No LLM provider configured");
  const ctl = new AbortController();
      ...
      const mapped = Object.values(pages)
        .map((p: any) => ({
          ...
          link:
            p?.fullurl ||
            (p?.title
              ? `https://en.wikipedia.org/wiki/${encodeURIComponent(String(p.title).replace(/\s+/g, "_"))}`
              : ""),
        }))
...
async function serperImage(q: string): Promise<string | null> {
  ...
  try {
    ...
    const ok = list.find(
      (it: { imageUrl?: string; imageWidth?: number }) =>
...
async function findImage(q: string | undefined): Promise<string | null> {
  ...
  const tries = [cleaned, cleaned.split(/\s+/).slice(0, 3).join(" ")].filter(
...
async function buildOutline(
  ...
  brandKit?: BrandKit,
): Promise<{ title?: string; subtitle?: string; language?: string; slides: RawSlide[] }> {
  const lengthRule =
...
- LANGUAGE & DIALECT (HIGHEST PRIORITY — STRICT, NON-NEGOTIABLE): The "Language hint" provided in the user message is AUTHORITATIVE. You MUST write ALL textual fields (title, subtitle, body, bullets, steps, events, quotes, kickers, big_label, stats labels) in EXACTLY that language and in the SAME dialect/register the user wrote their topic in. NEVER switch languages. If hint = "en" → output English only (no Arabic, no other scripts). If hint = "ar" → mirror the user's Arabic dialect exactly (Egyptian/مصري, Khaleeji/خليجي, Levantine/شامي, Maghrebi/دارجة, Iraqi, MSA — never default to MSA if user wrote in dialect). Same rule for every other language. The proper-noun subject of the deck (e.g. "ronaldo", "Tokyo") does NOT override the language hint. Only "image_query" stays English.`;
  ...
  if (!out) {
    console.warn(
      "[outline] schema validation failed",
      raw && typeof raw === "object" ? Object.keys(raw as object).slice(0, 5) : raw,
    );
  ...
  if (out && out.slides.length > 0) {
    return outline.slides.map((o, i) => {
      ...
      return {
        ...o,
        ...clean,
        image_query: (deep.image_query as string) || (o.image_query as string),
      };
    });
  }
...
function detailedObjects(value: unknown, minDescWords = 8): unknown[] {
  return Array.isArray(value)
    ? value.filter((x) => {
        ...
        return (
          words(item.title) >= 1 &&
          (words(item.desc) >= minDescWords || words(item.label) >= 1 || words(item.value) >= 1)
        );
      })
    : [];
}
...
function hasRichSlideContent(s: Record<string, unknown>): boolean {
  ...
  if (
    ["stat-cluster", "stat-circles", "kpi-strip", "big-number"].includes(layout) ||
    type === "stats"
  ) {
...
function guaranteeMinimumSlideContent(
  ...
  return slides.map((s, idx) => {
    ...
    if (
      ...
      if (stats.length < 3) {
        s.stats = ar
          ? [
              { label: "محور", value: String(idx + 1).padStart(2, "0") },
              { label: "نقاط داعمة", value: "5" },
              { label: "رسالة واضحة", value: "100%" },
            ]
          : [
              { label: "Angle", value: String(idx + 1).padStart(2, "0") },
              { label: "Support points", value: "5" },
              { label: "Clear message", value: "100%" },
            ];
      }
    ...
    if (["process", "step-vertical", "numbered-list"].includes(layout)) {
      ...
      if (current.length < 4) {
        const labels = ar
          ? ["الخلفية", "التفصيل", "الأثر", "الخلاصة"]
          : ["Context", "Detail", "Impact", "Takeaway"];
    ...
    if (["timeline", "timeline-horizontal", "story-rows"].includes(layout)) {
      ...
      if (current.length < 4) {
        const labels = ar
          ? ["البداية", "النقطة الفاصلة", "الانتشار", "الأثر"]
          : ["Start", "Turning point", "Reach", "Impact"];
        s.events = labels.map((eventTitle, i) => ({
          date: String(idx + i + 1).padStart(2, "0"),
          title: eventTitle,
          desc: fallback.bullets[i],
        }));
      }
      return s;
    }
    ...
    if (["comparison", "vs-split", "before-after", "table-compare"].includes(layout)) {
      ...
      if (detailedStrings(s.left_bullets).length < 4)
        s.left_bullets = mergeTextItems(s.left_bullets, fallback.bullets.slice(0, 4), 4);
      if (detailedStrings(s.right_bullets).length < 4)
        s.right_bullets = mergeTextItems(s.right_bullets, fallback.bullets.slice(1), 4);
      return s;
    }

    if (words(s.body) < 55) s.body = fallback.body;
    if (detailedStrings(s.bullets).length < 4)
      s.bullets = mergeTextItems(s.bullets, fallback.bullets, 4);
    if (words(s.subtitle) < 10) s.subtitle = fallback.subtitle;
    return s;
  });
}
...
const LAYOUT_FAMILY: Record<string, string> = {
  ...
  "diagonal-split": "split-image",
  "hero-image-right": "split-image",
  "hero-image-left": "split-image",
  "split-image-stack": "split-image",
  "split-image-bleed": "split-image",
  "image-full": "image-hero",
  "focus-image": "image-hero",
  "image-card-overlay": "image-hero",
  "image-quote-overlay": "image-hero",
  "image-top": "image-stack",
  "image-bottom": "image-stack",
  "image-strip-top": "image-stack",
  "image-strip-bottom": "image-stack",
  "image-side-card": "image-card",
  polaroid: "image-card",
  "image-with-stats": "image-card",
  "image-with-caption": "image-card",
  "magazine-cover": "editorial",
  centered: "centered-text",
  "centered-narrow": "centered-text",
  "centered-eyebrow": "centered-text",
  "drop-cap-paragraph": "centered-text",
  "left-aligned-hero": "hero-text",
  "right-aligned-hero": "hero-text",
  "title-with-rule": "hero-text",
  "asymmetric-title": "hero-text",
  "split-title-body": "hero-text",
  "indented-statement": "hero-text",
  definition: "definition",
  manifesto: "statement",
  "poster-typo": "statement",
  callout: "statement",
  "oversized-title": "statement",
  "framed-title": "statement",
  "stacked-statement": "statement",
  "pull-quote": "quote",
  "dialogue-block": "quote",
  "two-col": "columns",
  "three-col": "columns",
  "four-col": "columns",
  "two-col-divided": "columns",
  "three-col-numbered": "columns",
  "kanban-cols": "columns",
  bento: "bento",
  "masonry-cards": "bento",
  "asymmetric-bento": "bento",
  "feature-grid-2x2": "bento",
  "feature-grid-3x2": "bento",
  pillars: "cards",
  "icon-grid": "cards",
  "ribbon-cards": "cards",
  "vertical-cards": "cards",
  "horizontal-strips": "cards",
  "card-list": "cards",
  "tiled-grid": "cards",
  comparison: "compare",
  ...
  "stat-hero": "hero-stat",
  "trend-callout": "hero-stat",
  "stat-cluster": "stat-grid",
  "stat-circles": "stat-grid",
  "stat-pair": "stat-grid",
  "stat-grid-4": "stat-grid",
  "metric-card": "stat-grid",
  "ranking-list": "stat-grid",
  "kpi-strip": "stat-strip",
  "stat-row": "stat-strip",
  "percentage-bar": "stat-strip",
  process: "process",
  ...
  gallery: "gallery",
  "image-grid-2": "gallery",
  "image-grid-4": "gallery",
  "carousel-strip": "gallery",
  "image-mosaic-3": "gallery",
  "image-mosaic-5": "gallery",
  "image-quote-pair": "gallery",
  "image-stats-pair": "gallery",
  "image-grid-6": "gallery",
};
...
class SlidesDeadlineError extends Error {
  constructor() {
    super("Slides generation exceeded the safe time limit");
  }
}
...
function ensureDeckHasImmediateImages(deck: { slides?: unknown }, subject: string): void {
  const slides = Array.isArray((deck as { slides?: unknown[] }).slides)
...
Instruction: ${instruction}`;
  ...
  await narrate(
    `Write ONE short conversational sentence saying you'll start researching the topic.`,
    `Topic: ${subject}`,
  );

  const searchResults = isLongInput
    ? []
    : (await withBudgetTimeout(
        serperSearch(subject),
        Math.min(12_000, Math.max(1_000, timeRemaining() - 5_000)),
      )) || [];

  // Firecrawl scrape top 3 links for richer corpus (only when not using user material).
  send({ type: "phase", name: "findings" });
  ensureBudget();
  const scrapeCorpus = isLongInput
    ? ""
    : (await withBudgetTimeout(
        firecrawlEnrich(
        ...
        Math.min(15_000, Math.max(1_000, timeRemaining() - 5_000)),
      )) || "";
  ...
  await narrate(
    `Write 3-5 flowing sentences (no bullets, no markdown) sharing the most interesting facts you just discovered. Start with an enthusiastic word.`,
    findingsContext,
  );

  send({ type: "phase", name: "outline" });
  await narrate(
    `Write ONE short sentence saying you'll dig deeper and start structuring the deck.`,
    `Subject: ${subject}`,
  );
  ...
  // Deep-Research-style plan: emit the proposed deck plan BEFORE expanding it,
  // so the user can see what will be generated (slide titles + focus) up front.
  try {
    const planSlides = (outline.slides || []).map((s: Record<string, unknown>, i: number) => ({
      n: i + 1,
      title: (s.title as string) || `Slide ${i + 1}`,
      focus: (s.focus as string) || (s.subtitle as string) || "",
    ...
    send({
      ...
      subject,
      language: lang,
      total: planSlides.length,
      slides: planSlides,
    });
    // Narrate the plan in the user's language so chat UIs that don't render
    // the structured `plan` event still see the plan inline.
    const planSummary = planSlides
      .map((p) => `${p.n}. ${p.title}${p.focus ? ` — ${p.focus}` : ""}`)
      .join("\n");
    await narrate(
      `Present this as a short "Here's my plan" message in the user's language and dialect, then say you'll start writing each slide in depth. Keep it under 90 words. Use a numbered list of the slide titles (titles only, no focus).`,
      `Plan:\n${planSummary}`,
    );
  } catch (e) {
    console.warn("[slides] plan emit failed", e);
  }
  ...
  if (requestedCount && requestedCount >= 3 && deepSlides.length !== requestedCount) {
    ...
    const closing =
      [...deepSlides].reverse().find((s) => s.type === "closing") ||
      deepSlides[deepSlides.length - 1];
    ...
    if (middle.length >= needMiddle) {
      ...
      const fallback =
        ...
        ({ type: "content", title: subject } as (typeof middle)[number]);
      while (nextMiddle.length < needMiddle) {
        nextMiddle.push({
          ...fallback,
          title: `${(fallback as { title?: string }).title || subject} — ${nextMiddle.length + 1}`,
        } as typeof fallback);
      }
    }
  ...
  if (ENABLE_BLOCKING_SMART_IMAGES && timeRemaining() > 25_000) {
    const slidesNeedingImage = deepSlides.filter((s) => {
      ...
      const textOnly =
        ["callout", "manifesto", "pull-quote", "poster-typo", "definition", "quote"].includes(
          layout,
        ) || s.type === "quote";
      return !textOnly;
    });
    ...
    await Promise.all(
      slidesNeedingImage.map(async (s) => {
        try {
          if (!s.image) {
            const url = await withBudgetTimeout(
              smartImage(s as Parameters<typeof smartImage>[0], subject, usedUrls),
              8_000,
            );
          ...
          console.warn("[smartImage slide]", e);
        }
      }),
    );
  }
...
Return JSON: { "fills": [{ "i": <index>, "subtitle"?, "bullets"?, "body"?, "stats"?, "big_value"?, "big_label"?, "steps"?, "events"?, "left_title"?, "right_title"?, "left_bullets"?, "right_bullets"? }] } — one entry per input slide, same "i" indices.`;
      ...
      const fill = await withBudgetTimeout(
        aiJson<{ fills?: Array<Record<string, unknown> & { i?: number }> }>(
          [
            { role: "system", content: fillSys },
            { role: "user", content: fillUser },
          ],
          ROUTER_MODELS.slidesExpand,
        ),
        Math.min(18_000, Math.max(3_000, timeRemaining() - 5_000)),
      );
      ...
      deepSlides = postProcessDeck(
        deepSlides as Record<string, unknown>[],
        subject,
  ...
  deepSlides = postProcessDeck(
    guaranteeMinimumSlideContent(
      deepSlides as Array<Record<string, unknown>>,
      subject,
      outline.language || lang,
    ),
    subject,
  ) as typeof deepSlides;
  deepSlides = guaranteeMinimumSlideContent(
    deepSlides as Array<Record<string, unknown>>,
    subject,
    outline.language || lang,
  ) as typeof deepSlides;
  deepSlides = enforceDigitalOasisVariety(
    deepSlides as Array<Record<string, unknown>>,
    tplId,
  ) as typeof deepSlides;
  ...
  if (shouldCritique) {
    ...
    await narrate(
      `Write ONE short sentence saying you're reviewing the whole deck for quality and to avoid repetition.`,
    );
  }
...
Include every slide that can be meaningfully improved. Empty fixes array is fine only if the deck is genuinely flawless. Keep the user's language and dialect exactly. Up to 20 fixes.`;
      ...
      const review = await withBudgetTimeout(
        aiJson<{
          fixes?: Array<{ i?: number; title?: string; bullets?: string[]; subtitle?: string }>;
        }>(
          [
            { role: "system", content: reviewSys },
            { role: "user", content: reviewUser },
          ],
          ROUTER_MODELS.slidesCritic,
        ),
        Math.min(18_000, Math.max(3_000, timeRemaining() - 8_000)),
      );
...
Empty fixes only if every slide is genuinely publication-ready. Keep user's language/dialect exactly. Up to 15 fixes.`;
        ...
        timeRemaining() > 25_000
          ? await withBudgetTimeout(
              aiJson<{
                ...
                [
                  { role: "system", content: polishSys },
                  { role: "user", content: polishUser },
                ],
                ROUTER_MODELS.slidesCritic,
              ),
              Math.min(14_000, Math.max(3_000, timeRemaining() - 8_000)),
            )
          : null;
  ...
  const weakAfterGuarantee = (deepSlides as Array<Record<string, unknown>>).filter(
    (s) => !hasRichSlideContent(s),
  ).length;
  if (weakAfterGuarantee > 0)
    console.warn(`[quality] ${weakAfterGuarantee} slides still weak after final guarantee`);

  send({ type: "phase", name: "finalize" });
  if (timeRemaining() > 6_000) {
    await narrate(
      `Write ONE cheerful closing sentence telling the user the deck is ready. Mention there are ${deepSlides.length} slides.`,
      subject,
    );
  } else {
    sendNarrate(
      lang.startsWith("ar")
        ? `العرض جاهز وفيه ${deepSlides.length} شريحة.\n\n`
  ...
  const deckShape = planDeckShape({
    requested: requestedCount,
    durationMin: autoDuration,
  ...
  if (userId && SUPABASE_SERVICE_ROLE_KEY) {
    try {
      ...
      await sb.rpc("spend_credits_auto", {
        p_user_id: userId,
        p_amount: 2,
...
// ────────────────────────────────────────────────────────────────────────────
// Plus AI Presentations API branch (standard tier).
// Triggered when caller passes `provider: "plusai"`. Creates a background job,
// calls Plus AI, polls until done, uploads PPTX to the public `presentations`
// bucket, and writes `{ standardSlides: { ... } }` to job.output for the client.
// Docs: https://guide.plusai.com/ai-for-presentations/presentations-api
// ────────────────────────────────────────────────────────────────────────────

const PLUS_API_BASE = "https://api.plusdocs.com/r/v0";
const PPTX_MIME = "application/vnd.openxmlformats-officedocument.presentationml.presentation";

async function plusCreatePresentation(
  prompt: string,
  opts: { language?: string; numberOfSlides?: number },
) {
  const apiKey = Deno.env.get("PLUSAI_API_KEY");
  if (!apiKey) throw new Error("Missing PLUSAI_API_KEY secret");
  const body: Record<string, unknown> = { prompt };
  if (opts.language) body.language = opts.language;
  if (opts.numberOfSlides) body.numberOfSlides = opts.numberOfSlides;
  const resp = await fetch(`${PLUS_API_BASE}/presentation`, {
    method: "POST",
  ...
  if (!resp.ok) {
    ...
    throw new Error(`Plus AI create failed (${resp.status}): ${t.slice(0, 400)}`);
  }
  return (await resp.json()) as { pollingUrl: string; status: string };
}

async function plusPollPresentation(pollingUrl: string) {
  const apiKey = Deno.env.get("PLUSAI_API_KEY")!;
  const resp = await fetch(pollingUrl, { headers: { Authorization: `Bearer ${apiKey}` } });
  if (!resp.ok) {
    const t = await resp.text().catch(() => "");
    throw new Error(`Plus AI poll failed (${resp.status}): ${t.slice(0, 400)}`);
  }
...
async function runPlusAIJob(
  ...
  await writer.start({ phase: "create", status_text: "جاري إرسال الطلب…" });
  await writer.setProgress(5, "create");

  // Auto-detect language from topic so Arabic prompts always produce Arabic decks.
  const detectedLang = detectLanguage(payload.topic);
  const language = payload.language || detectedLang;

  // Bigger by default — user wants substantial decks, not tiny 8-slide stubs.
  // Plus AI cap is 30. Use 25 as the comfortable default.
  const numberOfSlides = Math.min(30, Math.max(8, payload.numberOfSlides || 25));

  // Force real stock photography (search-based) over AI-generated illustrations.
  const imageRule =
    "IMAGE RULES: Use ONLY real stock photographs from web/photo libraries (search-based, photojournalistic). " +
    "Do NOT use AI-generated illustrations, 3D renders, cartoon graphics, or synthetic art. " +
    "Every slide should feature an authentic photo relevant to the topic.";

  const langRule =
    language === "en"
      ? ""
      : `LANGUAGE: Write ALL slide content — titles, bullets, captions, speaker notes — strictly in ${language === "ar" ? "Arabic (العربية)" : language}. Do NOT translate to English under any circumstance.`;

  const stylePrefix = [payload.stylePrompt || "", imageRule, langRule]
    .filter(Boolean)
    .join("\n\n")
    .trim();
  const fullPrompt = stylePrefix ? `${stylePrefix}\n\nTopic: ${payload.topic}` : payload.topic;

  const created = await plusCreatePresentation(fullPrompt, {
    language,
    numberOfSlides,
  });
  await writer.setProgress(15, "polling");
  await writer.setStatusText("جاري توليد الشرائح بواسطة Plus AI…");

  let final: { url: string; slides: string[] } | null = null;
  for (let i = 0; i < 60; i++) {
    if (await writer.isCanceled()) throw new Error("Canceled by user");
    await new Promise((r) => setTimeout(r, 5000));
    const row = await plusPollPresentation(created.pollingUrl);
    if (row.status === "GENERATED" && row.url) {
      final = { url: row.url, slides: row.slides || [] };
      break;
    }
    if (row.status === "FAILED") throw new Error("Plus AI marked the presentation as FAILED");
    const pct = 15 + Math.min(70, Math.round((i / 60) * 70));
    await writer.setProgress(pct, "polling");
  }
  if (!final) throw new Error("Timed out waiting for Plus AI presentation");

  await writer.setProgress(88, "upload");
  await writer.setStatusText("جاري رفع الملف…");

  const pptxResp = await fetch(final.url);
  if (!pptxResp.ok) throw new Error(`Failed downloading PPTX (${pptxResp.status})`);
  const bytes = new Uint8Array(await pptxResp.arrayBuffer());

  const sb = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY!);
  const path = `${userId}/${writer.id}.pptx`;
  const { error: upErr } = await sb.storage.from("presentations").upload(path, bytes, {
    contentType: PPTX_MIME,
    upsert: true,
  });
  if (upErr) throw new Error(`Storage upload failed: ${upErr.message}`);
  const { data: pub } = sb.storage.from("presentations").getPublicUrl(path);

  const title = (final.slides[0] || payload.topic || "Presentation").slice(0, 120);
  await writer.setProgress(100, "done");
  await writer.complete({
    standardSlides: {
      title,
      templateName: payload.templateName || "Standard (Plus AI)",
      url: pub.publicUrl,
      colors: (payload.templateColors || ["#0a0a0a", "#5b8def"]) as [string, string],
      slides: final.slides,
...
/* ───────────────────────────────────────────────────────────────────
 * Merged endpoint: slides-message (action: "message").
 * Generates the short plan/summary chat blurb for a deck.
 * Kept here so it shares the same deployment slot as chat-slides-stream.
 * ─────────────────────────────────────────────────────────────────── */
async function handleSlidesMessage(body: Record<string, unknown>): Promise<Response> {
  const mode = String(body?.mode ?? "plan");
  const topic = String(body?.topic ?? "");
  const kind = String(body?.kind ?? "slides");
...
- Plain text only. No code fences.`;
  ...
  const res = await fetch(router.url, {
    ...
    body: JSON.stringify({
      model: router.mapModel(ROUTER_MODELS.slides),
      messages: [
        { role: "system", content: sys },
        { role: "user", content: user },
      ],
...
serve(async (req) => {
  ...
  // Merged sub-endpoint: short plan/summary message (was slides-message).
  if (body?.action === "message") return await handleSlidesMessage(body);
  ...
  // ── Plus AI branch (standard tier) — short-circuit the React-deck pipeline ──
  if (body.provider === "plusai" && userId) {
    const plusJobId = await createJob({
      userId,
      kind: "slides",
      input: {
        topic,
        templateId: typeof templateId === "string" ? templateId : "standard-professional",
        provider: "plusai",
      },
      meta: { provider: "plusai" },
      conversationId,
      messageId,
    });
    const tColors =
      Array.isArray(body.templateColors) && body.templateColors.length === 2
        ? ([String(body.templateColors[0]), String(body.templateColors[1])] as [string, string])
        : undefined;
    runInBackground(plusJobId, async (writer) => {
      await runPlusAIJob(writer, userId, {
        topic,
        language: typeof body.language === "string" ? body.language : undefined,
        numberOfSlides: Number.isFinite(body.numberOfSlides)
          ? Number(body.numberOfSlides)
          : undefined,
        stylePrompt: typeof body.stylePrompt === "string" ? body.stylePrompt : undefined,
        templateName: typeof body.templateName === "string" ? body.templateName : undefined,
        templateColors: tColors,
      });
    });
    return new Response(JSON.stringify({ jobId: plusJobId }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
  const rawId = typeof templateId === "string" ? templateId : "";
  const requestedTemplateId = REACT_TEMPLATES.has(rawId) ? rawId : "digital-oasis";
  const tplId = requestedTemplateId;
