...
export function useMediaUpload() {
  ...
  const upload = useCallback(async (file: File, opts: UploadOptions = {}): Promise<UploadedMedia> => {
    setUploading(true);
    setError(null);
    setProgress(0);
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("telegram-tasks-bot/storage-upload", {
        body: form,
      });
