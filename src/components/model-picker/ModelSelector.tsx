...
const CHAT_MODELS: ModelOption[] = [
  ...
  { id: "anthropic/claude-sonnet-4.6", name: "Claude Sonnet 4.6", credits: "", badges: ["PRO"] },
  { id: "anthropic/claude-opus-4.8", name: "Claude Opus 4.8", credits: "", badges: ["PRO"] },
];
...
export const getDefaultModel = (mode: ModelMode): ModelOption =>
  MODELS[mode]?.[0] || CHAT_MODELS[0];
        ...
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              className="absolute bottom-full mb-2 right-0 z-50 w-56 max-w-[calc(100vw-2rem)] rounded-xl border border-border bg-popover shadow-lg overflow-hidden"
            >
              {models.map((m) => (
                <button
                  key={m.id}
