...
interface Props {
  provider: ProviderKey | "gemini" | "qwen" | "flux" | "tongyi" | "sora" | "hailuo" | "firefly" | "grok";
  className?: string;
  size?: number;
}
...
export default function ProviderLogo({ provider, className = "", size = 18 }: Props) {
  ...
  if (!entry) {
    ...
    return (
      <span
        className={className}
        style={{ width: size, height: size, borderRadius: 999, background: "currentColor", opacity: 0.4, display: "inline-block" }}
      />
    );
  }
