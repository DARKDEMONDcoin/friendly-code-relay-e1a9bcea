...
import { Pin, Plus, PanelLeft, LogIn, Cloud } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { AnimatePresence, motion } from "framer-motion";
import { useSidebarCollapsed } from "@/hooks/useSidebarCollapsed";
import MegsyStar from "@/components/files/MegsyStar";
import megsyLogo from "@/assets/megsy-icon-transparent.png";
...
const prefetchRoute = (path: string) => {
  ...
  if (key.startsWith("/chat")) task = import("@/pages/chat/ChatPage");
  else if (key.startsWith("/settings/referrals")) task = import("@/pages/billing/ReferralsPage");
  else if (key.startsWith("/settings")) task = import("@/pages/settings/SettingsPage");
  else if (key.startsWith("/pricing")) task = import("@/pages/marketing/PricingPage");
  else if (key.startsWith("/auth")) task = import("@/pages/auth/AuthPage");
...
const AppSidebar = ({
  ...
  const sectionAccent = useMemo(
    () => sectionAccentFor(location.pathname, currentMode),
  ...
  const navigateSmoothly = useCallback(
    (path: string, options?: NavigateOptions) => {
      ...
      startTransition(() => navigate(path, options));
    },
    [closeInline, navigate, onClose],
  );
  ...
  const navItems: Array<{
    ...
    { label: "Home", Icon: HomeIcon, path: "/", match: (p: string) => p === "/" || p.startsWith("/chat") },
    {
      label: "Earn",
      Icon: EarnIcon,
  ...
  const innerContent = (
    ...
    >
      ...
      >
        ...
        {inline && (
          <button
            ...
            style={{ color: TEXT, border: `2px solid ${BORDER}`, backgroundColor: SURFACE }}
      ...
      >
        {navItems.map(({ label, Icon, path, match, isNew }) => {
          ...
          return (
            ...
            >
              <Icon size={18} strokeWidth={2.2} />
              <span className="text-[14px]" style={{ fontWeight: active ? 800 : 700 }}>{label}</span>
              {isNew && (
                <span
      ...
      {location.pathname.startsWith("/media") && (
        ...
        >
          {(() => {
            const active = location.pathname.startsWith("/gallery");
            if (isCollapsed) {
              return (
                <button
        ...
        {!currentUserId ? (
          ...
          >
            ...
            <button
              ...
              className="shrink-0 inline-flex items-center gap-1 h-8 px-2.5 rounded-full text-[12px] transition active:translate-x-[1px] active:translate-y-[1px] active:shadow-none"
          ...
          {navItems.map(({ label, Icon, path, match }) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-[14.5px]" style={{ fontWeight: active ? 800 : 700 }}>{label}</span>
              </button>
            );
          })}
          {location.pathname.startsWith("/media") && (
            ...
            >
              ...
              <span className="text-[14.5px]" style={{ fontWeight: 700 }}>Cloud</span>
            </button>
          )}
        </div>
      ...
      {open && (
        <>
          <motion.div
            ...
            className="fixed inset-0 z-popover bg-foreground/25 backdrop-blur-md cursor-pointer"
            onClick={onClose}
