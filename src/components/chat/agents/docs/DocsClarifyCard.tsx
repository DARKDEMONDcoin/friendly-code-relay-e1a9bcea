...
export default function DocsClarifyCard({ reason, questions, ui, onSubmit, busy }: Props) {
  ...
  const contentDir =
    detectUiLang(reason, q.label, q.help ?? "", ...(q.options ?? [])) === "ar" ? "rtl" : "ltr";
      ...
      return (
        <div className="flex items-center gap-3 p-2 rounded-xl border border-border/60 bg-background/60">
          ...
          <button
            type="button"
            onClick={onClear}
            className="w-8 h-8 rounded-full hover:bg-muted/60 flex items-center justify-center text-muted-foreground hover:text-foreground"
            title={tt("clear")}
          >
      ...
      <div className="space-y-2">
        ...
        >
          ...
          <span className="text-[12.5px] font-medium">
            {uploading ? tt("uploading") : tt("uploadCta")}
          </span>
        </button>
        <div className="flex items-center gap-2">
        ...
        {q.options.map((opt) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${selected ? "border-indigo-600 bg-indigo-600 dark:border-indigo-400 dark:bg-indigo-500" : "border-border"}`}
              >
                {selected && <Check className="w-3 h-3 text-foreground" strokeWidth={3.5} />}
              </span>
            </button>
          );
        })}
        ...
        {q.options.map((opt) => {
          ...
          return (
            ...
            >
              <span className="font-semibold text-[13px] leading-snug">{opt}</span>
              <span
                className={`shrink-0 w-5 h-5 rounded-md border-2 flex items-center justify-center transition-colors ${isSel ? "border-indigo-600 bg-indigo-600 dark:border-indigo-400 dark:bg-indigo-500" : "border-border"}`}
              >
                {isSel && <Check className="w-3 h-3 text-foreground" strokeWidth={3.5} />}
              </span>
            </button>
          );
        })}
