...
const AgentStatusLine = ({ searchStatus, toolActivity, userText }: AgentStatusLineProps) => {
  ...
  const ar = useMemo(() => {
    ...
    return false;
  }, [userText, searchStatus, toolActivity?.target]);

  // Decide active phase
  let phase: Phase;
  if (toolActivity && toolActivity.status === "running") phase = "tool";
  else if (lastDoneTool && Date.now() - doneShownAt < 900) phase = "tool_done";
  else if (searchStatus?.trim()) phase = "tool"; // backend-provided custom status renders like a tool row
...
const ToolIcon = ({
  ...
  if (!meta) return <Wrench className="w-4 h-4 text-foreground/70" />;
  if (meta.slug && !failed) {
    return (
      <img
        ...
        alt=""
        className={
          "w-4 h-4 dark:invert " + (muted ? "opacity-50" : "opacity-90")
        }
  ...
  const Comp =
    (meta.lucide && (Lucide as any)[meta.lucide]) ||
    (Lucide as any).Wrench;
  return (
    <Comp className={"w-4 h-4 " + (muted ? "text-foreground/50" : "text-foreground/80")} />
  );
};
