    ...
    <AnimateDropdownMenu open={isOpen} onOpenChange={setOpen}>
      <AnimateDropdownMenuTrigger asChild>
        <button
          ...
          className="inline-flex h-9 items-center gap-2 rounded-full pl-1 pr-3 text-[12.5px] font-black text-brand-parchment bg-surface-1 border-2 border-surface-4 active:translate-x-[2px] active:translate-y-[2px] transition"
        >
          <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-brand-action border-2 border-brand-ink">
            <Settings2 className="h-3.5 w-3.5 text-brand-ink" />
          </span>
      ...
      <AnimateDropdownMenuContent
        ...
        align="end"
        sideOffset={10}
        className="z-[61] w-[min(360px,calc(100vw-16px))] rounded-[24px] border-[2.5px] border-brand-ink bg-surface-1 p-4 shadow-[4px_4px_0_rgba(59,130,246,0.22)]"
      >
      ...
      {showHeader && (
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="text-[13px] font-black tracking-tight text-brand-parchment">
              {isVideo ? "Video settings" : "Image settings"}
            </div>
            <div className="text-[10.5px] text-brand-muted font-semibold mt-0.5 truncate">{summary}</div>
          </div>
          <span className="shrink-0 text-[9.5px] uppercase tracking-[0.22em] text-brand-ink font-black px-2.5 py-1 rounded-full bg-brand-mint border-2 border-brand-ink">
            {isVideo ? "Video" : "Image"}
          </span>
        </div>
      )}
          ...
          {RATIOS.map((r) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center gap-1 h-[58px] rounded-2xl border-2 transition-all active:translate-x-[2px] active:translate-y-[2px] ${
                  active
                    ? "bg-brand-action text-brand-ink border-brand-ink shadow-[3px_3px_0_rgba(59,130,246,0.28)]"
                    : "bg-surface-3 border-surface-4 text-brand-parchment/70 hover:bg-[#242424]"
                }`}
              >
                <span
                  className={`rounded-[3px] border-2 ${active ? "border-brand-ink" : "border-brand-parchment/45"}`}
        ...
        <div className="flex flex-col">
          ...
          {isVideo && (
            <AttachTile
              ...
              hint="Interpolate motion between two frames"
            />
          )}
          {isVideo && (
            <AttachTile
              title="Audio"
              hint="Lip-sync or score (Wan 2.5)"
            />
          )}
        </div>
      </Collapsible>
...
function Collapsible({
  ...
  return (
    <div className="min-w-0 border-b border-white/[0.06] last:border-b-0">
      <button
        type="button"
        onClick={() => onToggle(id)}
        aria-expanded={open}
      ...
      >
        <span className="truncate text-[11px] font-black uppercase tracking-[0.14em] text-brand-muted">
          {label}
        </span>
        <span className="flex items-center gap-1.5 shrink-0">
          {value && (
              <span className="text-[11.5px] font-black text-brand-parchment tabular-nums truncate max-w-[160px]">
              {value}
            </span>
          )}
          <ChevronDown
            className={`h-3.5 w-3.5 text-brand-action transition-transform ${open ? "rotate-180" : ""}`}
          />
        </span>
      </button>
...
function Segmented({
  ...
  return (
    <div className="inline-flex w-full p-1 rounded-xl bg-surface-3 border-2 border-surface-4">
      {items.map((it) => {
        ...
        return (
          <button
            ...
            className={`flex-1 h-8 inline-flex items-center justify-center rounded-lg text-[12px] font-semibold transition-all active:scale-95 ${
              active
                ? "bg-brand-action text-brand-ink border border-brand-ink"
                : "text-brand-parchment/70 hover:text-brand-parchment"
            }`}
          >
            {it.label}
          </button>
...
function AttachTile({
  ...
  return (
    <div className="flex items-start gap-2.5 py-2 px-3 rounded-xl bg-surface-3 border-2 border-surface-4 mb-1.5 last:mb-0">
      {icon && <div className="mt-0.5 shrink-0 text-white/55">{icon}</div>}
      <div className="min-w-0">
        <div className="text-[12.5px] font-black text-brand-parchment leading-tight">{title}</div>
        <div className="text-[10.5px] text-brand-muted font-semibold leading-snug mt-0.5">{hint}</div>
      </div>
    </div>
  );
}
