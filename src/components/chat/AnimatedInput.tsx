...
const AnimatedInput = ({
  ...
  return (
    <div className="relative">
      ...
      {headerSlot && (
        <div className="mb-2 flex justify-start pointer-events-auto px-1">{headerSlot}</div>
      )}
      ...
      <div className="md:rounded-ios-lg md:p-[1px] md:bg-border/60">
        <div className="chat-composer-frame chat-mobile-input-glow composer-card pointer-events-auto rounded-[1.35rem] px-4 pt-1 pb-2 relative z-10 bg-background md:bg-card md:rounded-[25px] md:px-3.5 md:pt-3.5 md:pb-3 md:border md:border-foreground/[0.02]">
          ...
          <div className="px-1">
              ...
              {isEditing && (
                ...
                >
                  <div className="flex items-center pt-1 pb-1.5">
                    <span className="inline-flex items-center gap-1.5 rounded-full pl-3 pr-1 py-1 text-[12px] font-semibold bg-primary/15 text-primary">
            ...
            <div className="relative">
              {!value && displayedPlaceholder && (
                <div
                  aria-hidden
                  className="pointer-events-none absolute inset-0 flex items-start px-1 pt-2 text-[15.5px] md:text-sm text-foreground/50 leading-relaxed overflow-hidden"
                >
          ...
          >
            ...
            {/* Mobile-only modes toggle: shows the mode pill bar above the
              composer. Appears after the bar auto-hides on first send. */}
            {modesToggleVisible && onToggleModes && (
              <motion.button
                ...
                className="md:hidden shrink-0 w-9 h-9 flex items-center justify-center rounded-full bg-transparent border-0 text-foreground/85 hover:text-foreground hover:bg-foreground/10 transition-colors"
            ...
            <AnimatePresence mode="popLayout" initial={false}>
              {isLoading ? (
                <motion.button
                  ...
                  whileTap={{ scale: 0.9 }}
                  onClick={onCancel}
                  className="shrink-0 w-9 h-9 md:h-10 md:w-10 flex items-center justify-center rounded-full bg-transparent text-foreground"
