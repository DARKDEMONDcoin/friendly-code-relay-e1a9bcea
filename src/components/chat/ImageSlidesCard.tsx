...
const PdfPreviewModal = ({ url, chatName, onClose }: ModalProps) => {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-toast bg-black/95 backdrop-blur flex flex-col"
    >
      <header className="flex items-center gap-3 px-4 py-3 shrink-0">
        ...
        <span className="ms-auto rounded-full bg-white/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-foreground/75 shrink-0">
          Featured
        </span>
      </header>
        ...
        {error && (
          <div className="flex flex-col items-center gap-3 text-foreground/70 mt-20 max-w-md mx-auto text-center">
            ...
            <a
              href={url}
              download
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white text-foreground font-semibold text-[13px]"
            >
      ...
      <footer className="shrink-0 px-4 py-4 flex items-center justify-center gap-3" dir="ltr">
        <a
          href={url}
          download={fileName}
          className="inline-flex items-center justify-center h-10 px-6 rounded-full bg-white text-foreground text-sm font-medium tracking-wide hover:bg-white/90 transition"
        >
          Download
        </a>
        <button
          type="button"
          onClick={handleShare}
          className="inline-flex items-center justify-center h-10 px-6 rounded-full bg-white/10 hover:bg-white/20 text-foreground text-sm font-medium tracking-wide transition"
        >
          Share
        </button>
      </footer>
