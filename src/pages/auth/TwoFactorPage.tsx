...
import {
  INK, MINT, LAVENDER, PINK, SURFACE, BORDER, TEXT, MUTED, PAGE_BG,
...
const TwoFactorPage = () => {
  ...
  const verifyEnroll = async () => {
    ...
    try {
      ...
      const { error: vErr } = await supabase.auth.mfa.verify({
        factorId, challengeId: ch.id, code: otp,
      });
      if (vErr) throw vErr;
  ...
  const Content = () => (
    <>
      {step === "loading" && (
        <div className="text-center py-12 text-sm" style={{ color: MUTED }}>Loading…</div>
      )}
      ...
      {step === "status" && (
        <div className="space-y-4">
          ...
          >
            ...
            <p className="mt-3 text-[11px] uppercase tracking-wider" style={{ fontWeight: 800, color: INK, opacity: 0.7 }}>
              Status
            </p>
            <p className="mt-1 text-[18px]" style={{ fontWeight: 900, color: INK }}>
              {factors.length ? "Enabled" : "Disabled"}
            </p>
            <p className="mt-2 text-[13px] max-w-sm" style={{ fontWeight: 600, color: INK, opacity: 0.8 }}>
              {factors.length
          ...
          {factors.length ? (
            <button
              ...
              style={{ backgroundColor: PINK, color: INK, fontWeight: 900, border: `2.5px solid ${INK}`, boxShadow: `4px 4px 0 ${INK}` }}
            >
            ...
            <button
              ...
              disabled={busy}
              className="w-full py-3.5 rounded-full text-[14px] disabled:opacity-50"
              style={{ backgroundColor: MINT, color: INK, fontWeight: 900, border: `2.5px solid ${INK}`, boxShadow: `4px 4px 0 ${INK}` }}
            >
      ...
      {step === "enroll" && (
        <div className="space-y-4">
          ...
          >
            ...
            {qr && (
              <div className="mt-3 inline-block p-3 rounded-2xl bg-white" style={{ border: `2px solid ${INK}` }}>
                <img src={qr} alt="2FA QR" width={200} height={200} />
              </div>
            )}
            <p className="mt-3 text-[11px] uppercase tracking-wider" style={{ color: MUTED, fontWeight: 800 }}>
              Or enter this key manually
            </p>
            <button
              onClick={copySecret}
              className="mt-1 inline-flex items-center gap-2 px-3 py-2 rounded-xl text-[12px] font-mono"
              style={{ backgroundColor: PAGE_BG, border: `2px solid ${BORDER}`, color: TEXT, fontWeight: 700 }}
            >
              {secret}
  ...
  if (!isMobile) {
    return (
      <DesktopSettingsLayout title="Two-Factor Authentication" subtitle="Protect your account with an authenticator app">
    ...
    <div className="relative min-h-[100dvh] overflow-y-auto" style={{ backgroundColor: PAGE_BG, color: TEXT }}>
      ...
      >
        <div className="max-w-lg mx-auto px-5 flex items-center justify-between py-3 safe-top">
          ...
          <h1 className="text-[17px]" style={{ fontWeight: 900, letterSpacing: "-0.02em" }}>Two-Factor Auth</h1>
          <div className="w-10" />
        </div>
      </header>
