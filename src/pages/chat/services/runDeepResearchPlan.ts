...
export async function runDeepResearchPlan(args: RunDeepResearchPlanArgs): Promise<void> {
  const {
    userInput, localTurnId, conversationId, conversationPromise,
    researchDepth, chatUserId, userName,
    setMessages, setActiveResearchJobId,
    setIsLoading, setIsThinking, resetToolUi,
    saveMessage, ownInsertedIdsRef, presenceChannelRef,
  } = args;
  ...
  try {
    ...
    try {
      ...
      if (aId) {
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.clientId === assistantClientId ? ({ ...m, id: aId } as Message) : m,
          ),
        );
      }
    } catch (e) {
