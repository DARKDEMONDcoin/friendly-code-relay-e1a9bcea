...
export interface RunSlidesTurnArgs {
  ...
  insertAssistantNarration: (cid: string | null | undefined, text: string, clientId?: string) => Promise<void>;
...
export async function runSlidesTurn(args: RunSlidesTurnArgs): Promise<void> {
  const {
    userInput, localTurnId, chatUserId, slidesTemplate,
    setChatMode, setSearchEnabled, setIsLoading, setIsThinking, resetToolUi,
    setMessages, setSearchStatus,
    createOrUpdateConversation, saveMessage, ownInsertedIdsRef,
    fetchSlidesNarration, insertAssistantNarration,
    clearSlidesTimeout, slidesTimeoutsRef, slidesGenerationTokenRef,
  } = args;

  const slidesRequestToken = ++slidesGenerationTokenRef.current;
  const isSlidesRequestCancelled = () =>
    slidesGenerationTokenRef.current !== slidesRequestToken;
    ...
    await new Promise<void>((resolve) => {
      ...
      unsub = subscribeJob(jobId, {
        onProgress: (_p, phase) => {
          ...
          setSearchStatus(lbl || `Working: ${phase}…`);
        ...
        onDone: async () => {
          ...
          if (finalStandardSlides) {
            const ss = finalStandardSlides as {
              title: string; templateName: string; url: string;
              colors: [string, string]; slides?: string[]; slideCount?: number;
            };
            setMessages((prev) =>
              prev.map((m) =>
                m.clientId === `assistant-${localTurnId}` ||
