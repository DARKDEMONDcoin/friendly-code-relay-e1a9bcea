    ...
    <>
      ...
      >
        ...
        <div className="border-t border-border/30">
          <button
            onClick={() => { p.setShareMode("private"); p.setGeneratedShareUrl(null); }}
          ...
          <button
            onClick={() => { p.setShareMode("public"); if (!p.generatedShareUrl) p.handleCreateShareLink("public"); }}
          ...
          {p.shareMode === "public" ? (
            p.generatedShareUrl ? (
              <div className="mx-auto flex items-center justify-center gap-2 max-w-full w-fit rounded-xl liquid-glass-button px-3 py-2.5 overflow-hidden">
                ...
                <button onClick={p.handleCopyShareLink} className="shrink-0 p-1.5 rounded-lg liquid-glass-hover transition-colors" aria-label="Copy">
      ...
      <ChatDesktopDialog open={p.isRenaming} onOpenChange={p.setIsRenaming}>
        ...
        <div className="flex justify-end gap-2 px-5 pb-5 border-t border-border/30 pt-4">
          <button onClick={() => p.setIsRenaming(false)} className="px-4 py-2 rounded-xl text-sm text-muted-foreground hover:text-foreground liquid-glass-hover transition-colors">
            Cancel
          </button>
          <button onClick={p.handleRename} className="px-4 py-2 rounded-xl text-sm font-semibold bg-primary text-primary-foreground hover:opacity-90 transition-opacity">
            Save
          </button>
        </div>
      </ChatDesktopDialog>
        ...
        <div className="px-5 pt-4 pb-5">
          {p.inviteLink ? (
            <div className="flex items-center gap-2 w-full rounded-xl liquid-glass-button px-3 py-2.5">
              ...
              />
              <button onClick={p.handleCopyInviteLink} className="shrink-0 p-1.5 rounded-lg liquid-glass-hover transition-colors" aria-label="Copy">
                <Copy className="w-3.5 h-3.5 text-foreground" />
              </button>
            </div>
          ) : (
