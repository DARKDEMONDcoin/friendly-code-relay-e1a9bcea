    ...
    >
      {msg.role === "assistant" && msg.operatorRunId ? (
        <Suspense fallback={null}>
          <OperatorInlineBubbleLazy
            runId={msg.operatorRunId}
            onDismiss={dismissOperatorRun}
          />
        </Suspense>
      ) : (
        <ChatMessage
          ...
          onEditUserMessageAt={
            msg.role === "user" ? handleEditUserMessageAt : undefined
          }
          ...
          readers={
            (msg.id ? readersByMessageId[msg.id] || EMPTY_READERS : EMPTY_READERS) as any
          }
          showReaders={
            hasMembers &&
            msg.role === "user" &&
          ...
          <div className="px-3 md:px-12 mt-3">
            <div className="rounded-2xl border border-border/50 bg-card/60 backdrop-blur-xl p-4 max-w-xl">
              ...
              <div className="text-[12px] text-muted-foreground mb-3">
                Slide generation was interrupted before completing. You can
                regenerate it.
              </div>
              <button
                onClick={() => {
