...
export function useChatSocialActions(params: {
  ...
  const handleCreateShareLink = useCallback(
    async (modeOverride?: "private" | "public") => {
      ...
      if (mode === "public") {
        ...
        setGeneratedShareUrl(`${window.location.origin}/share/${newShareId}`);
      } else {
        await supabase
          .from("conversations")
  ...
  }, [conversationId, generatedShareUrl, isShared, shareMode, setShareDialogOpen, handleCreateShareLink]);
  ...
  }, [conversationId, setInviteDialogOpen, setInviteLink, setInviteEmail, setMembers]);
  ...
  const handleSendInviteEmail = useCallback(async () => {
    ...
    setInviteLoading(false);
  }, [conversationId, inviteEmail, inviteLink, setInviteEmail, setInviteLink, setInviteLoading]);
  ...
  const handleGenerateInviteLink = useCallback(async () => {
    ...
    setInviteLink(`${window.location.origin}/invite/${(data as any).invite_token}`);
    setInviteLoading(false);
  }, [conversationId, setInviteLink, setInviteLoading]);
