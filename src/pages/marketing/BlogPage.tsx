...
const BlogPage = () => {
  ...
  const headingHero = lang === "ar"
    ? { kicker: "مجلة Megsy", h1: "نكتب عن العمل، لا عن الضجيج.", sub: "أدلة عملية حول الذكاء الاصطناعي للصور والفيديو والكتابة." }
    : { kicker: "The Megsy Journal", h1: "Writing about the work, not the hype.", sub: "Practical guides on AI image generation, video, prompt engineering, and the modern creator stack." };
  ...
  return (
    <div data-theme="dark" dir={langMeta.dir} lang={lang} className="min-h-dvh overflow-x-hidden bg-background text-foreground">
      ...
      <Helmet htmlAttributes={{ lang, dir: langMeta.dir }}>
        ...
        {BLOG_LANGS.map((l) => (
          <link key={l.code} rel="alternate" hrefLang={l.code} href={`${SITE_ORIGIN}${l.code === "en" ? "/blog" : `/${l.code}/blog`}`} />
        ))}
      ...
      <main className="px-4 pt-32 pb-24 mx-auto max-w-5xl">
        <header className="mb-10 text-center">
          <p className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground mb-3">{headingHero.kicker}</p>
          <h1 className="font-display text-4xl md:text-6xl font-bold tracking-tight">{headingHero.h1}</h1>
          <p className="mt-6 text-muted-foreground text-base md:text-lg max-w-2xl mx-auto">{headingHero.sub}</p>
        </header>
