...
import {
  CREDITS_PER_SIGNUP, COMMISSION_PCT,
...
export default function ProgramTab() {
  const steps = [
    { icon: <Link2 className="h-5 w-5" strokeWidth={2.5} />, t: "Share your link", d: "One link, two rewards" },
    { icon: <UserPlus className="h-5 w-5" strokeWidth={2.5} />, t: "Friend joins Megsy AI", d: `They get ${CREDITS_PER_SIGNUP} free credits` },
    { icon: <CreditCard className="h-5 w-5" strokeWidth={2.5} />, t: "They subscribe", d: "Any plan or top-up" },
    { icon: <Zap className="h-5 w-5" strokeWidth={2.5} />, t: `You earn ${COMMISSION_PCT}% cash + ${CREDITS_PER_SIGNUP} credits`, d: "Lifetime, every payment" },
  ];
        ...
        {perks.map((p) => (
          ...
          >
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[18px]" style={{ fontWeight: 900 }}>{p.label}</span>
            </div>
            <div className="mt-1 text-[12px]" style={{ fontWeight: 700, opacity: 0.7 }}>{p.sub}</div>
          </div>
        ))}
      </div>

      {/* Section title */}
      <h2 className="px-1 text-[20px]" style={{ fontWeight: 900, letterSpacing: "-0.02em", color: TEXT }}>
        How it works
      </h2>
        ...
        {steps.map((s, i) => (
          ...
          >
            ...
            <span
              ...
              style={{ backgroundColor: YELLOW, color: INK, fontWeight: 900, border: `1.5px solid ${INK}` }}
            >
              {i + 1}
            </span>
          </li>
