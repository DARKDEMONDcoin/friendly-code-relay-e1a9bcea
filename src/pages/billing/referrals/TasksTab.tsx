...
import {
  EmptyState, RewardTask, useReferrals,
...
const resolveTaskIconSrc = (task: { task_key?: string; action_url?: string | null; action_type?: string }): string | null => {
...
const TaskIcon = ({ task }: { task: { task_key?: string; action_url?: string | null; action_type?: string } }) => {
  const src = resolveTaskIconSrc(task);
  if (!src) return <Gift className="h-5 w-5" strokeWidth={2.5} />;
  return <img src={src} alt="" width={512} height={512} loading="lazy" className="h-10 w-10 object-contain" />;
};
...
export default function TasksTab() {
  ...
  if (tasks.length === 0) {
    return <div className="mt-6"><EmptyState title="No tasks available" hint="Check back soon for new tasks." /></div>;
  }
    ...
    <div className="mt-6 space-y-5" style={{ color: TEXT }}>
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="max-w-[60%]">
            ...
            <h1 className="mt-3 text-[26px] leading-[1.05]" style={{ fontWeight: 900, letterSpacing: "-0.02em", color: INK }}>
              Tap, do, claim!
            </h1>
        ...
        <div className="mt-4" style={{ color: INK }}>
          <div className="flex items-center justify-between text-[12px]" style={{ fontWeight: 800 }}>
            <span>Progress</span>
            <span className="tabular-nums">{completed} / {tasks.length}</span>
          </div>
          <div
      ...
      <ul className="space-y-3">
        {tasks.map((t: RewardTask, i) => {
          ...
          const progress = t.action_type === "invite_friends"
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-[14.5px] leading-tight" style={{ fontWeight: 800, color: TEXT }}>
                  {t.title}
                </p>
                <p className="mt-1 inline-flex items-center gap-1 text-[12px]" style={{ fontWeight: 700, color: MUTED }}>
                  <span
                    className="rounded-full px-2 py-0.5"
                    style={{ backgroundColor: YELLOW, border: `1.5px solid ${INK}`, fontWeight: 900, color: INK }}
                  >
                    +{t.reward_credits}
                  </span>
                  <span>credits</span>
                  {t.action_type === "invite_friends" && <span>· {progress}/{t.target_count}</span>}
                </p>
              </div>
              <button
                onClick={() => claimTask(t)}
              ...
              >
                {isDone ? (
                  <span className="inline-flex items-center gap-1"><Check className="h-3.5 w-3.5" strokeWidth={3} /> Done</span>
                ) : locked ? (
                  <span className="inline-flex items-center gap-1"><Lock className="h-3.5 w-3.5" strokeWidth={3} /> Locked</span>
