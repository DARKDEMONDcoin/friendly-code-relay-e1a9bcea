...
import { CartoonHero } from "@/components/settings/CartoonSettingsShell";
import { INK, YELLOW, SURFACE, BORDER, TEXT, PAGE_BG } from "@/pages/billing/ReferralsPage";
import skillsSticker from "@/assets/settings/skills-sticker.png";
...
export default function SkillsSettingsPage() {
  ...
  const mobileHeader = (
    ...
    >
      <div className="max-w-3xl mx-auto px-5 flex items-center justify-between py-3 safe-top">
        <button
        ...
        <h1 className="text-[17px]" style={{ fontWeight: 900, letterSpacing: "-0.02em", color: TEXT }}>Skills</h1>
        <input
          ref={fileInputRef}
          type="file"
          accept=".zip"
        ...
        <div ref={addMenuRef} className="relative">
          <button
            ...
            className="grid h-10 w-10 place-items-center rounded-full active:translate-x-[1px] active:translate-y-[1px] active:shadow-none transition disabled:opacity-50"
            style={{ backgroundColor: YELLOW, border: `2px solid ${INK}`, color: INK, boxShadow: `2px 2px 0 ${INK}` }}
          >
          ...
          <AnimatePresence>
            {addMenuOpen && (
              <motion.div
                ...
                className="absolute right-0 top-full mt-2 z-50 w-52 rounded-2xl overflow-hidden p-1"
              >
                <button
                  onClick={() => { setAddMenuOpen(false); navigate("/settings/skills/new"); }}
                  className="w-full text-left text-[13px] px-3 py-2.5 rounded-xl hover:bg-accent/70 transition-colors"
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        {s.description && (
                          <p className="text-[12.5px] text-muted-foreground line-clamp-2 mt-0.5 leading-snug">
                            {s.description}
                          </p>
                        )}
            ...
            <div className="space-y-2">
              {librarySkills.map((s) => {
                ...
                return (
                  <div
                    key={s.id}
                    className="p-4 rounded-2xl border border-border/50 bg-card hover:border-border transition-all flex items-start gap-3"
                  >
                    <div className="flex-1 min-w-0">
  ...
  return (
    <div className="min-h-dvh" style={{ backgroundColor: PAGE_BG, color: TEXT }}>
      ...
      <main className="max-w-3xl mx-auto px-4 pb-24">
        <CartoonHero
          sticker={skillsSticker}
          bg={YELLOW}
          title="Your Skills"
          subtitle="Tune the experts Megsy can switch into."
        />
  ...
  return (
    <div className="flex flex-wrap gap-1 mt-2">
      {shown.map((t) => (
        <span
          key={t}
          className="text-[10.5px] px-2 py-0.5 rounded-full bg-muted/70 text-muted-foreground"
        >
          {t}
        </span>
      ))}
