      ...
      <div className="max-w-lg mx-auto pb-16 px-5">
        <div className="flex items-center justify-between py-4">
          <button
            ...
            className="w-9 h-9 flex items-center justify-center rounded-xl hover:bg-muted/50"
          >
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="mt-2 mb-6 p-5 rounded-3xl bg-gradient-to-br from-primary/10 to-accent/5 border border-border/40">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Control AI permissions inside its own environment: Browser, Shell, Dynamic agents, and
              spend limits.
            </p>
          ...
          <Row
            icon={ShieldCheck}
            title="Ask me before sensitive actions"
            desc="Payments, emails, deleting files, public publishing."
          >
            <Switch
              checked={settings.ask_before_sensitive}
              onCheckedChange={(v) => save({ ask_before_sensitive: v })}
            />
          </Row>
          <Row
            icon={ShieldCheck}
            title="Ask me before any action"
            desc="Strict mode — the AI asks before every tool."
          >
            <Switch
              checked={settings.ask_before_anything}
              onCheckedChange={(v) => save({ ask_before_anything: v })}
            />
          </Row>

          {/* Capabilities */}
          <p className="text-[11px] uppercase tracking-wider text-muted-foreground mb-1 px-1 mt-6">
            Capabilities
          </p>
          <Row
            icon={Globe}
          ...
          >
            <Switch
              ...
              onCheckedChange={(v) => save({ allow_browser_automation: v })}
            />
          </Row>
          <Row
            icon={Terminal}
            title="Free shell inside the environment"
            desc="bash, apt, python, ffmpeg. Danger — for advanced users only."
          >
            <Switch
              checked={settings.allow_free_shell}
              onCheckedChange={(v) => save({ allow_free_shell: v })}
            />
          </Row>
          <Row
            icon={Bot}
            title="Dynamic agents"
            desc="AI generates a new agent (e.g. marketing agent) on demand and saves it."
          >
            <Switch
              checked={settings.allow_dynamic_agents}
              onCheckedChange={(v) => save({ allow_dynamic_agents: v })}
            />
          </Row>
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">
                  {settings.max_parallel_agents} Agents working together
                </p>
              </div>
            </div>
            <Slider
          ...
          <div className="py-4 border-b border-border/40">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">
                  ${(settings.budget_cap_cents / 100).toFixed(2)} max
                </p>
              </div>
            </div>
            <Slider
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground">
                {agentCount} Specialized agent generated automatically
              </p>
            </div>
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground">
                Every action the AI performed — exportable
              </p>
            </div>
            <span className="text-muted-foreground/40">›</span>
          </button>
