...
export default function WorkspaceTasksPage() {
  ...
  const createTask = async (status: Status) => {
    ...
    if (draft.assignee_id && draft.assignee_id !== me) {
      try {
        await supabase.functions.invoke("report-error", { headers: { "x-fn": "workspace-notify" }, body: { type: "task_assigned", workspace_id: id, assignee_id: draft.assignee_id, title },
        });
      } catch {}
    }
  };
    ...
    <div className="min-h-dvh bg-background">
      <header className="sticky top-0 z-10 bg-background/95 backdrop-blur border-b border-border/40">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center gap-3">
          <button
            onClick={() => navigate(`/settings/workspaces/${id}`)}
                  ...
                  {showCreate === col.id && (
                    <div className="mb-3 p-3 rounded-lg bg-background border border-border/60 space-y-2">
                      ...
                      <div className="flex gap-2">
                        <Button size="sm" onClick={() => createTask(col.id)} className="flex-1">
                          Add
                        </Button>
                        <Button size="sm" variant="ghost" onClick={() => setShowCreate(null)}>
                          Cancel
                        </Button>
                      </div>
                    </div>
                    ...
                    {grouped[col.id].map((t) => {
                      ...
                      return (
                        <button
                          ...
                          className="w-full text-left p-3 rounded-lg bg-card border border-border/60 hover:bg-muted/30 transition"
                        >
                          <div className="flex items-start justify-between gap-2">
                            <p className="text-sm font-medium flex-1">{t.title}</p>
                            <span
                              className={`text-[10px] px-1.5 py-0.5 rounded ${PRIORITY_COLOR[t.priority]}`}
                            >
                              {t.priority}
                            </span>
                          </div>
...
function TaskDrawer({ task, allTasks, members, onClose, onDelete }: any) {
  ...
  const addComment = async () => {
    ...
    for (const t of targets) {
      try {
        await supabase.functions.invoke("report-error", { headers: { "x-fn": "workspace-notify" }, body: {
            type: "task_comment",
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm" onClick={onClose}>
      <div
        className="absolute right-0 top-0 bottom-0 w-full max-w-md bg-background border-l border-border overflow-y-auto"
            ...
            <div className="space-y-1">
              {subs.map((s: Task) => (
                <button
                  key={s.id}
                  onClick={() => toggleSub(s)}
                  className="w-full flex items-center gap-2 p-2 rounded hover:bg-muted/40 text-left text-sm"
                >
                  {s.status === "done" ? (
